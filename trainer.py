...
def ddpg(
    ...
    print(
        now,
        "- Training {}".format(
            "single agent" if single_agent else "{} agents".format(20)
        )
        + " for max {} episodes. Target score to reach is {}".format(
            num_episodes, target
        ),
    )
    ...
    for episode in range(1, num_episodes + 1):
        ...
        states = (
            env_info.vector_observations[0]
            if single_agent
            else env_info.vector_observations
        )
        ...
        if mean_score >= target:
            ...
            print(
                "Target score of {0} has been reached. Saving model to {1}".format(
                    target, saved_model
                )
            )
...
def test(agent_: ContinuousControlAgent, env_settings, single_agent, filename):
    ...
    while True:
        ...
        # get the next state
        reward = (
            env_info.rewards[0] if single_agent else env_info.rewards
        )  # get the reward
        done = (
            env_info.local_done[0] if single_agent else env_info.local_done
    ...
    print(
        "Score for {} agent(s): {}".format(
            num_agents, score_(score) if single_agent else score_(np.mean(score))
        )
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="""This script trains a continuous control agent using the 
    Deep Deterministic Policy Gradient (DDPG) algorithm"""
    )
    parser.add_argument(
        "agents", choices=["single", "multi"], help="Number of agents to train"
    )
    ...
    parser.add_argument(
        "--updateFrequency", default=2, help="Frequency with which to update models"
    )
    parser.add_argument(
        "--mode",
        default="train",
    ...
    memory = ReplayBuffer(
        action_size, config.BUFFER_SIZE, config.BATCH_SIZE, random_seed=0
    )
    agent = ContinuousControlAgent(
        state_size, action_size, 0, memory=memory, update_frequency=args.updateFrequency
    )
